...
def untar_file(tar_file,tmp_dir):
    try:
        subprocess.check_call("tar -zxvf -C %s %s"%(tmp_dir,tar_file),shell=True)
...
def fold_and_score_pipeline(data):
    ...
    for pointing in data["data"]["pointings"]:
       ...
       for beam in pointing["beams"]:
           ...
           df = pd.read_csv('%s/*good_cands_to_fold_with_beam.csv'%(tmp_dir)) 


           # Select only candidates with corresponding beam id and snr cutoff
           single_beam_cands = df[df['beam_id']==beam_ID & df['snr'] > processing_args['snr_cutoff']]
           single_beam_cands = single_beam_cands.sort('snr',ascending=False)  
           ...
           xml_file = single_beam_cands_to_fold_limited['file'].to_numpy()[0] # Choose first element. If filtered right, there should be just one xml filename throughout!
           ...
           for x in range(batches):
              ...
              for i in range(start,end):
                  ...
                  try:
                      process = subprocess.Popen("prepfold -ncpus 1 -nsub 64 -mask %s -noxwin -topo -p %s -pd %s -dm %s %s -o %s"%(mask_path,str(folding_packet['period']),str(folding_packet['pdot']),str(folding_packet['dm']),input_filenames,output_name),shell=True,cwd=output_path)
           ...
           make_tar_file(output_dir,tmp_dir,tar_name) 
           log.info("Tarred")
