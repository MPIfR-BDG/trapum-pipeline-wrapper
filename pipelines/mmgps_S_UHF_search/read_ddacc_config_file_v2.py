...
import pandas as pd 
import configparser, subprocess, argparse, sys
...
parser.add_argument('-c', '--config_file', help='Configuration file based on metadata and search range', type=str,  default="sample_ddacc_v2.cfg")
...
def main():
    ...
    for search_type in sections:
        ...
        if not (search_type == "PRESTO_PATH" or search_type == "OBSERVATION_METADATA"):
            ''' Configparser by default reads keys as string, converting this to list of tuples'''

            segment_list = literal_eval(config[search_type]['LIST_FRACTIONAL_SEGMENTS'])

            # Check for duplicate entries in segment list
            duplicate_segments = list(set([element for element in segment_list if segment_list.count(element) > 1]))

            if duplicate_segments:
                raise ValueError('You idiot! You have duplicates %s in your search segment list for the search category: %s' %(duplicate_segments, search_type))
