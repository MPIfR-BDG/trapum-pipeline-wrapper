...
class PikaChannel(object):
    def __init__(self, host, port, user, password, vhost):
        ...
        self._parameters = pika.ConnectionParameters(
            host, port, vhost, self._credentials)
        ...
        with self._channel_manager as channel:
            if hasattr(messages, "__iter__") and not isinstance(messages, (str, bytes)):
                for message in messages:
                    log.info("Publishing message '{}' to queue '{}'".format(
                        message, self._queue_params["queue"]))
                    channel.basic_publish(
                        exchange='', routing_key=self._queue_params["queue"],
                        body=message, properties=pika.BasicProperties(
                            delivery_mode=2, priority=priority))
            else:
                log.info("Publishing message '{}' to queue '{}'".format(
                    messages, self._queue_params["queue"]))
                channel.basic_publish(
                    exchange='', routing_key=self._queue_params["queue"],
                    body=messages, properties=pika.BasicProperties(
                        delivery_mode=2, priority=priority))
...
def add_pika_producer_opts(parser):
    ...
    parser.add_option('', '--log_level',dest='log_level',type=str,
                      help='Logging level for pikaprocess logger',
                      default="INFO")
