...
def peasoup_pipeline(data):
    ...
    for pointing in data["data"]["pointings"]:
        for beam in pointing["beams"]:
            ...
            if max_dm - min_dm > dm_range_lim: 
                log.info("Searching in DM splits of %f"%dm_range_lim)
                dm_segs = int(math.ceil((max_dm-min_dm)/dm_range_lim))
                for i in range(dm_segs):
                    if i ==dm_segs -1:
                        dm_start = min_dm +i*dm_range_lim
                        dm_end = max_dm 

                    else:
                        dm_start = min_dm + i*dm_range_lim
                        dm_end = min_dm + (i+1)*dm_range_lim
                                        
                    output_dir = output_dir +'dm_%f_%f'%(dm_start,dm_end)
                    peasoup_script = "peasoup -k Ter5_16apr_rfifind.badchan_peasoup -z trapum_latest.birdies  -i %s --dm_start %.2f --dm_end %.2f --limit %d  -n %d  -m %.2f  --acc_start %.2f --acc_end %.2f  --fft_size %d -o %s"%(merged_file,dm_start,dm_end,processing_args['candidate_limit'],int(processing_args['nharmonics']),processing_args['snr_threshold'],processing_args['start_accel'],processing_args['end_accel'],fft_size,output_dir)
