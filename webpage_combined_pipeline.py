...
def peasoup_pipeline(data):
    ...
    for pointing in data["data"]["pointings"]:
        for beam in pointing["beams"]:
            ...
            # Run peasoup
            peasoup_script = "peasoup -z trapum_latest.birdies  -i %s --dm_start %.2f --dm_end %.2f --limit %d  -n %d  -m %.2f  --acc_start %.2f --acc_end %.2f  --fft_size %d -o %s"%(subbanded_file,processing_args['min_dm'],processing_args['max_dm'],processing_args['candidate_limit'],int(processing_args['nharmonics']),processing_args['snr_threshold'],processing_args['start_accel'],processing_args['end_accel'],fft_size,output_dir)
            call_peasoup(peasoup_script)

            # Remove tmp files after searching
            tmp_files=[]
            #tmp_files.append(merged_file)
            tmp_files.extend((merged_file,subbanded_file,iqred_file))
