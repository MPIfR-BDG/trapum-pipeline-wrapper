...
def peasoup_pipeline(data):
    ...
    for pointing in data["data"]["pointings"]:
        for beam in pointing["beams"]:
            ...
            # DM split if needed  
            max_dm = float(processing_args['max_dm'])
            min_dm = float(processing_args['min_dm'])
            dm_list = np.loadtxt(processing_args['dm_list'],dtype=float)
            ...
            if len(dm_list) > 100:  
                ...
                for i in range(dm_segs):
                    if i ==dm_segs -1:
                        dm_list = dm_list[i*dm_trial_lim:]
                    ...
                    output_dir = output_dir + '/' + os.path.basename(dm_list_name)
                    peasoup_script = "peasoup -k Ter5_16apr_rfifind.badchan_peasoup -z trapum_latest.birdies  -i %s --dm_list %s --limit %d  -n %d  -m %.2f  --acc_start %.2f --acc_end %.2f  --fft_size %d -o %s"%(merged_file,dm_list_name,processing_args['candidate_limit'],int(processing_args['nharmonics']),processing_args['snr_threshold'],processing_args['start_accel'],processing_args['end_accel'],fft_size,output_dir)
                ...
                peasoup_script = "peasoup -k Ter5_16apr_rfifind.badchan_peasoup -z trapum_latest.birdies  -i %s --dm_list %s --limit %d  -n %d  -m %.2f  --acc_start %.2f --acc_end %.2f  --fft_size %d -o %s"%(merged_file,processing_args['dm_list'],processing_args['candidate_limit'],int(processing_args['nharmonics']),processing_args['snr_threshold'],processing_args['start_accel'],processing_args['end_accel'],fft_size,output_dir)
                call_peasoup(peasoup_script)
